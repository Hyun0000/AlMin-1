...
public class LoginInterceptor extends HandlerInterceptorAdapter {// Interceptor를 통한 로그인 처리
	/*
	 * Interceptor의 특징 1. prehandle에서 return값이 false일 경우 다음으로 넘어가지 않고 끝나게 된다. 2.
	 * posthandle의 경우 Controller에서 Exception이 발생 할 경우 posthandle로 요청이 넘어오지 않는다. 3.
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		if (requestURL.startsWith("/members/mypage")) {
			if (loginInfo.equals(null)) {
				System.out.println("세션타입: " + loginInfo.getSessionType());
				System.out.println("마이페이지 접근 실패");
	...
	/*
	 * prehandle -> 로그인 여부 체크할 때 많이 씀. posthandle -> 어떤 유저가 어떤 컨트롤러 타는지 검사할때 많이 씀.
	 * after (쓸일별로없다.)
	 */
	@Override
